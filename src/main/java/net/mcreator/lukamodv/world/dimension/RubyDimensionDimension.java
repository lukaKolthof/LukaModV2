...
package net.mcreator.lukamodv.world.dimension;
...
import net.mcreator.lukamodv.item.RubyDimensionItem;
import net.mcreator.lukamodv.block.UltraRubyBlockBlock;
...
public class RubyDimensionDimension extends LukamodV2ModElements.ModElement {
	@ObjectHolder("lukamod_v2:ruby_dimension_portal")
	public static final CustomPortalBlock portal = null;
	public RubyDimensionDimension(LukamodV2ModElements instance) {
		super(instance, 14);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(UltraRubyBlockBlock.block).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("lukamod_v2:ruby_dimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("lukamod_v2:ruby_dimension"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("lukamod_v2:ruby_dimension")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == UltraRubyBlockBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? UltraRubyBlockBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, UltraRubyBlockBlock.block.getDefaultState(), 3);
					}
				}
			}
